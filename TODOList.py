...
# Читает все строки как список
    ...
    if len(tasks) == 0:
        ...
        print("\nВаши Задачи: ")
# Делает нумерацию (1 - купить хлеб, 2 - сделать дз)
...
def delete_task():
    ...
    if 1 <= num <= len(tasks):
# Удаляет задачу по номеру, num -1 потому что список начинается с 0, а пользователь вводит с 1
...
# Перезапись файла, записывает обновленный список     
...
# task.strip() - убирает \n (перенос строки)
    ...
    if 1 <= num <= len(tasks):
# Берем строку и заменяем с [ ] на [x]
...
def search_task():
    ...
    with open("tasks.txt", "r") as file:
        tasks = file.readlines()
# Если ничего не нашли - покажем сообщение        
    found = False      
    
    for task in tasks:
# Проверяем: есть ли слово внутри строки
        if search in task:
